(function (root) {
  ...
  function processManualFix(composer) {
    ...
    chooseCorrection(original, settings).then(function (decision) {
      ...
      api.undo.showNotice("Clarify failed · Original kept", "error");
  ...
  // The on-page button is opt-in (settings.showButton); submission correction
  // runs regardless, so this only controls the manual-control surface.
  function syncFixButton() {
    if (state.settings && state.currentComposer && state.settings.showButton &&
        api.settings.isSiteEnabled(state.settings, state.siteId)) {
      api.fixButton.attach(state.currentComposer, processManualFix);
    } else {
  ...
  function watchComposers() {
    return api.composer.watchComposer(state.siteId, function (composer) {
